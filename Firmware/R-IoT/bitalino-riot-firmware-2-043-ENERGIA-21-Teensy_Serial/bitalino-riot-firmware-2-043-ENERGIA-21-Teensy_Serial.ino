...
//HC-SR04 definitions
#define trigPin 4         //trigger pin for the ultrasonic
#define echoPin 3         //echo pin
#define MAXRANGE 400        //max range in cm, making this higher makes the results unreliable
#define MINRANGE 3          //minimal range in cm, only change this if you know your distance can't be less than this value.

char serial_str[40];
String serial_values[3];
...
OscBuffer SerialData; //Teensy edit
...
//    ok = BITalino.version(verStr, sizeof verStr);    // get device version string
    ok = true; //WP, Always start microcontroller in any case, Standalone is now false by default
...
//      UdpPacket.endPacket(); 
//
      //Serial data in - always stream regardless of frame number
      if (Serial1.available() && !StandAloneMode)
      {
        Serial.println("Streaming From Teensy Buffer");
...
//
//        //Teensy Serial Data
        TempInt = (short)FrameAmount;
        ShortToBigEndian(pData, TempInt);
        pData += sizeof(int); 
         
        for(k=0 ; k < 10 ; k++)
        {
          String serialValStr = serial_values[1];
          TempInt = (short)serialValStr.toInt();
          ShortToBigEndian(pData, TempInt);
          pData += sizeof(int); 
        } 
...
//        String serial_values[3];
        ...
        String FrameAmountStr = serial_values[0];
        FrameAmount = FrameAmountStr.toInt();
        Serial.println(FrameAmount);
      }
//    }
  }
